...
export default function ForecastChart({ forecast }: ForecastChartProps) {
  ...
  const sliced = view === "all" ? raw : raw.slice(0, Number(view));
  ...
  const avgCases = Math.round(
    data.reduce((s, d) => s + d.cases, 0) / (data.length || 1),
  );
    ...
    <div className="glass border border-[var(--border)] p-6 rounded-2xl slide-up flex flex-col gap-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-[var(--text-muted)] mt-1">
            Starting from {formatDate(forecast.last_known_date)} ·{" "}
            {forecast.horizon_days}-day window
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          {
            label: "Last Known",
            value: forecast.last_known_cases.toLocaleString(),
            color: "text-amber-400",
          },
          {
            label: "Avg / Day",
            value: formatNumber(avgCases),
            color: "text-blue-400",
          },
      ...
      <p className="text-[10px] text-[var(--text-muted)] text-center">
        ⚠️ Predictions are model-generated estimates and should not be used for
        clinical decisions.
      </p>
    </div>
  );
